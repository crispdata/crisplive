     ...
     * Return ERR if it could not fork() the first time, OK otherwise. */
{
    ...
    case 0:
        /* child */
        {
            ...
            case 0:
                ...
                /* the grand child does not use this pipe: close it */
                xclose_check(&(pipe_pid_fd[0]), "grand child's pipe_pid_fd[0]");
            ...
            default:
                ...
                /* read the ack from our parent */
                /* NOTE: we ignore errors -- the call is blocking and the fact
                 *  we get one should be enough
                 * to know we can close the pipe now */
                read_pipe(pipe_pid_fd[0], &pipe_ack, 1);
#ifdef CHECKRUNJOB
    ...
    default:
        ...
        /* send an ACK to the child, so as it knows it can safely close the pipe now */
        ret = write_pipe(pipe_pid_fd[1], "A", 1);
        if (ret != OK) {
            if (ret == ERR)
                error
                    ("run_job(): parent: Could not tell child we read the pipe");
            else {
                errno = ret;
                error_e
                    ("run_job(): parent: Could not tell child we read the pipe");
