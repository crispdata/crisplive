...
$(document).ready(function () {
    ...
    $('.project-warning-sweetalert').on('click', function () {
        swal("", "Event name already existed!", "warning");
    });
    ...
    $(document).on('keypress', '.contact_name_admin', function () {
        ...
        $(".contact_name_admin").autocomplete({

            source: function (request, response) {
                $.ajax({
                    type: 'post',
                    url: baseUrl + 'contact/getadmin',
                    dataType: "json",
                    data: {'contact': request.term, '_csrf-backend': csrf_token},
                    success: function (resultData) {
                        $.each(resultData, function (i, e) {
    ...
    $(document).on('focusout', '.contact_name_admin', function () {
        var contact_name = $(this).parent().find('.contact_name_admin').val();
        var contact_id = $(this).parent().find('.contact_id').val();
        var contact_stat_div = $(this);
        ...
        $.ajax({
            type: 'post',
            url: baseUrl + 'contact/is-exist-admin',
            dataType: "json",
            data: {'id': contact_id, 'contact_name': contact_name, '_csrf-backend': csrf_token},
            success: function (resultData) {
                if (resultData == 'success') {
                    ...
                    contact_stat_div.parent().parent().find('.contact_stat').empty();
                    contact_stat_div.parent().parent().find('.contact_stat').append('<a id = "c_contact_modal" class="btn-floating btn-small waves-effect waves-light blue modal-trigger c_contact_modal" href = "#add-contact"><i class="material-icons">add</i></a>');
                    if ($.trim(contact_stat_div.parent().parent().find('.contact_name_admin').val()) != "") {
                        $('#c_contact_modal').trigger('click');
                        $('.contact_stat').removeClass('current-modal');
                        contact_stat_div.parent().parent().find('.contact_stat').addClass('current-modal');
                    }
                    contact_stat_div.parent().parent().find('.contact_name_admin').val("");
                }
            }
        });
    ...
    $("#add-role-project-admin").on("click", function () {

        var role_html_content = '<div class="row"><div class="input-field col s3 m3"><input type="hidden" class = "contact_id validate" name = "CreateProject[ContactId][]"><input type="text" class = "contact_name_admin validate" name = "ContactId[]"><label>Contact Name</label></div><a style = "display:none" id = "c_contact_modal" class="modal-trigger" href = "#add-contact"></a><div class="input-field col s2 m1 contact_stat"></div><div class="input-field col s3 m2"><input type="text" name = "CreateProject[RoleName][]" class="validate"><label>Assign Role</label></div>';
    ...
    $('#data-table').on('click', '.proj-delete', function (e) {
            ...
            $.ajax({
                ...
                success: function (resultData) {
                    ...
                    if (contact_resp.status == "success") {
                        ...
                        $('.current-modal').parent().find('.contact_name_admin').val('');
                        $('.current-modal').parent().find('.contact_id').val('');
                        $('.current-modal').parent().find('.contact_name').val(contact_resp.name);
                        $('.current-modal').parent().find('.contact_name_admin').val(contact_resp.name);
    ...
    $('#email-me').on('click', function () {
        ...
        $.ajax({
            type: 'post',
            url: baseUrl + 'report/report-content-email',
            dataType: "json",
            data: {project_id: project_id, '_csrf-backend': csrf_token},
            success: function (resultData) {
                //report_html = resultData;
                $('#show-email-me-email').html(resultData);
                $('#show-email-me-email').show();
            }
        });
    });
    ...
    $('#email-others').on('click', function () {
        ...
        $('#show-email-me').empty();
        $('#show-email-me').hide();
        $('#report-tab-2').removeClass('white');
        $('#report-tab-2').addClass('blue');
        $('#report-tab-3').removeClass('blue');
        $('#report-tab-3').addClass('white');
        var project_id = $("input[name='GenerateReport[project_id]']").val();
        $.ajax({
            type: 'post',
            url: baseUrl + 'report/report-content-email',
    ...
    $('#check-none-pages').on('click', function () {
        console.log($(this).prev().is(':checked'));
...
function requestmessage(pid, uid, mid) {
    $.ajax({
        type: 'post',
        url: baseUrl + 'message/requestmessage',
        dataType: "json",
        data: {'pid': pid, 'UserId': uid, 'ThreadId': mid, '_csrf-backend': csrf_token},
        success: function (resultData) {
            if (resultData == 'success') {
...
function requestmessagepage(itemid, pageid, pid, uid, mid) {
    $.ajax({
        type: 'post',
        url: baseUrl + 'message/requestmessage',
        dataType: "json",
        data: {'itemid': itemid, 'pageid': pageid, 'pid': pid, 'UserId': uid, 'ThreadId': mid, '_csrf-backend': csrf_token},
        success: function (resultData) {
            if (resultData == 'success') {
                $("#id" + itemid + "").removeAttr('class');
                $("#id" + itemid + "").removeAttr('onclick');
                $("#id" + itemid + "").attr('class', 'waves-effect waves-light btn green modal-trigger');
                document.getElementById("id" + itemid + "").innerHTML = "Sent";
                //swal("Success !!!", "Request successfully sent!", "success");
                //window.location.href = window.location.href;
            }
        }
...
function requestmessageproject(pageid, pid, uid, mid) {
    $.ajax({
        type: 'post',
        url: baseUrl + 'message/requestmessage',
        dataType: "json",
        data: {'pageid': pageid, 'pid': pid, 'UserId': uid, 'ThreadId': mid, '_csrf-backend': csrf_token},
        success: function (resultData) {
            if (resultData == 'success') {
                $("#id" + pageid + "").removeAttr('class');
                $("#id" + pageid + "").removeAttr('onclick');
                $("#id" + pageid + "").attr('class', 'waves-effect waves-light btn green m-b-xs add-contact');
                document.getElementById("id" + pageid + "").innerHTML = "Sent";
                //swal("Success !!!", "Request successfully sent!", "success");
                //window.location.href = window.location.href;
            }
        }
...
function acceptrequest(rid, uid, tid, pid) {
    $.ajax({
        type: 'post',
        url: baseUrl + 'message/acceptrequest',
        dataType: "json",
        data: {'rid': rid, 'pid': pid, 'uid': uid, 'tid': tid, '_csrf-backend': csrf_token},
        success: function (resultData) {
            if (resultData == 'success') {
                //swal("Success !!!", "Request successfully sent!", "success");
                window.location.href = window.location.href;
...
function rejectrequest(mid) {
    $.ajax({
        type: 'post',
        url: baseUrl + 'message/rejectrequest',
        dataType: "json",
        data: {'mid': mid, '_csrf-backend': csrf_token},
        success: function (resultData) {
            if (resultData == 'success') {
                //swal("Success !!!", "Request successfully sent!", "success");
                window.location.href = window.location.href;
...
function openpopup(page, item) {

    window.location.href = window.location.href + '#itemthrd' + item + '';
    location.reload();
}
